...
Use when implementing features using any Liveblocks packages, for
...
number of reference files are available in`references/` directory. You
must read list below, and load relevant files as needed. e.g.,
`add-user-information` is available in`references/add-user-information.md`
file.
...
- `ai-as-a-collaborator`: Use agentic workflows to allow AI to collaborate like
  human inside your Liveblocks application.
- `ai-chats-with-tools-knowledge-components`: Set up and troubleshoot chats with
  ...
  AI as collaborator, as it isn't workflow based.
- `auth-endpoint-callback`: Use callback function to authenticate users,
  instead of passing string to`authEndpoint`. Useful for passing custom
  headers to your back end, and for preventing automatic reconnection when
  token is invalid.
...
- `compartmentalize-resources-with-organizations`: Use organizations to create
  ...
  notifications, comment threads, and more. filter by organization when
...
- `create-custom-realtime-multiplayer`: Use Liveblocks Storage to build fully
  ...
  for one-off realtime features too, like simple properties in document.
- `create-custom-text-editor-toolbar`: Set up styled toolbar, static or
...
- `dark-mode-styles`: import CSS styling for dark mode themes.
- `develop-and-test-locally`: Set up local dev server, and use it for
...
- `exhaustive-deps-with-usemutation`: Prevent and fix stale-closure bugs with
  `useMutation` by configuring`react-hooks/exhaustive-deps` ESLint rule.
...
- `handling-unstable-connections`: add fallbacks and error messages when
  users have poor network conditions.
- `log-out-of-liveblocks`: Rarely useful, but helpful in specific SPA situations
  where 't navigate page to log out.
- `multiplayer-react-flow`: Use Liveblocks React Flow package to create
  multiplayer diagrams with cursors and more.
- `multiple-text-editors`: Add multiple Tiptap or BlockNote editors to same
  page. Optionally use Storage to hold their field IDs.
- `offline-support-in-text-editors`: Give your text editors illusion of
  quicker load time.
- `override-css-variables`: Add custom styles to default components by
...
- `remove-liveblocks-branding`: Liveblocks logo badge appears in bottom
  right of screen, this is how to remove it.
...
- `send-comment-notification-emails`: Send email notifications to users when
  ...
  triggered when user is mentioned, or has participated in thread.
- `smoother-realtime-updates`: Make presence and storage run at higher frame
  rate, appearing more smooth. Using this option, updates can be received more
  frequently.
- `suspense-vs-regular-hooks`: read this when using any Liveblocks
...
- `url-params-in-room-id`: Use URL params in room IDs to create rooms, and
  incorporate document titles in URL.
- `utility-components`: Import ready-made emoji picker, or human-readable
...
Some files link to markdown files in Liveblocks docs, e.g.:

```
https://liveblocks.io/docs/concepts.md
```

When linking users to these pages, remove`.md` from link, so they can view
full content:
...
Always check if we provide package for your technology. e.g., if you're
using React Flow, use **`@liveblocks/react-flow`**.
...
- **`@liveblocks/redux`**: Sync Liveblocks room state with Redux store.
...
If users want to set up new technology, first check following
resources to see if package exists:
